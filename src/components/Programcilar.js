...
export default function Programcilar() {
  ...
  let [öneCikanProgramci, setöneCikanProgramci] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    return programciListesi[öneCikanProgramci-1].isim;
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: öneCikanProgramci == null ? 'royalblue': 'gold', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programciListesi.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setöneCikanProgramci(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
